...
def test(account='test_client@xoopit.com', secret='lSp7nnziZJMvl9Bw72Gyy4')
    ...
    r = c.get_documents('test', '#.#:' + docid, {}, 'cq.test.contact')
    assert( 'non-200: ' + JSON.generate(r) ) { (r['STATUS'] == 200) }
    ...
    r = c.get_documents('test', '*', {:fields =>'cq.test.*', :sort => ['^', '#.#']}, 'cq.test.contact')
    assert( 'non-200: ' + JSON.generate(r) ) { (r['STATUS'] == 200) }
    ...
    r = c.delete_documents('test', 'name:Steve', {:sort => '-#.createdTime', :limit => 2}, 'cq.test.contact')
    assert( 'non-200: ' + JSON.generate(r) ) { (r['STATUS'] == 200) }
    assert( 'should be 2 doc ids: %d' % r['result'].size ) { (r['result'].size == 2) }

    #r = c.count_documents('test', 'cq.test.contact', 'name:Steve')
    r = c.count_documents('test', 'name:Steve', 'cq.test.contact')
    assert( 'non-200: ' + JSON.generate(r) ) { (r['STATUS'] == 200) }
    ...
    r = c.get_documents('test', '*', {:fields => 'cq.test.*', :sort => ['^', '#.#'], :fieldmode => 'short'}, 'cq.test.contact')
    assert( 'non-200: ' + JSON.generate(r) ) { (r['STATUS'] == 200) }
    r['result'].each { |d|
        d.each { |k,v|
            assert( 'keys should be short, no dots' ) { (k.index('.') == nil) }
            assert( 'keys should be short' ) { (k[/^cq.test/] == nil) }
        }
    }
    ...
    assert( 'adding documents changed overall count' ) { (count == c.count_documents('test', '*', '*')['result']) }
